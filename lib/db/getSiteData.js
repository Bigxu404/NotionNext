...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  if (!pageRecordMap || Object.keys(pageRecordMap).length === 0) {
  ...
  // 调试：如果 rawMetadata 不存在，尝试查找匹配的 block
  if (!rawMetadata) {
    const blockKeys = Object.keys(block)
    console.warn(`[Notion] block[${pageId}] not found. Available block keys (${blockKeys.length}):`, blockKeys.slice(0, 5))
    // 尝试用不带横线的 ID 查找
    const plainId = pageId.replace(/-/g, '')
    for (const key of blockKeys) {
      if (key.replace(/-/g, '') === plainId) {
        rawMetadata = block[key]?.value
        console.log(`[Notion] Found block using alternative key: ${key}`)
        break
      }
    }
    // 如果仍然找不到，尝试取第一个 block
    if (!rawMetadata && blockKeys.length > 0) {
      const firstKey = blockKeys[0]
      const firstBlock = block[firstKey]?.value
      if (firstBlock?.type === 'collection_view_page' || firstBlock?.type === 'collection_view' || firstBlock?.collection_id) {
        rawMetadata = firstBlock
        console.log(`[Notion] Using first block as fallback: ${firstKey}, type: ${firstBlock?.type}`)
      }
    }
  }

  // 兼容性优化：支持 page 类型的数据库（Notion 新版 Wiki 或增强型数据库布局）
  const isDatabase =
    rawMetadata?.type === 'collection_view_page' ||
    rawMetadata?.type === 'collection_view' ||
    (rawMetadata?.type === 'page' && rawMetadata?.collection_id) ||
    // 进一步兼容：如果 collection 数据存在，即使 block type 不匹配也认为是数据库
    (rawMetadata && pageRecordMap.collection && Object.keys(pageRecordMap.collection).length > 0)

  if (!isDatabase) {
    console.error(`pageId "${pageId}" is not a database, type: ${rawMetadata?.type}`)
    console.error(`[Notion] block keys:`, Object.keys(block).slice(0, 5))
    console.error(`[Notion] has collection:`, !!pageRecordMap.collection, Object.keys(pageRecordMap.collection || {}).length)
    return EmptyData(pageId)
  }
  let collection = Object.values(pageRecordMap.collection || {})[0]?.value?.value || Object.values(pageRecordMap.collection || {})[0]?.value || {}

  let collectionId = rawMetadata?.collection_id || Object.keys(pageRecordMap.collection || {})[0]
  if (collectionId && pageRecordMap.collection && pageRecordMap.collection[collectionId]) {
      collection = pageRecordMap.collection[collectionId].value?.value || pageRecordMap.collection[collectionId].value;
  }
  
  // Fallback to try and find the schema if we are reading a page that represents a view or db instead of the database root directly
  let schema = collection?.schema || collection?.value?.schema || {};
  if (Object.keys(schema).length === 0 && pageRecordMap.collection) {
     for (const key of Object.keys(pageRecordMap.collection)) {
         const cVal = pageRecordMap.collection[key]?.value?.value || pageRecordMap.collection[key]?.value;
         if (cVal?.schema) {
             schema = cVal.schema;
             break;
         }
  ...
  if (!schema || Object.keys(schema).length === 0) {
      console.error("[获取数据]未能找到schema，会导致所有属性无法解析！");
  }
...
function handleDataBeforeReturn(db) {
  ...
  // 确保所有页面都有 href 字段，防止 Link 组件收到 undefined
  const ensureHref = pages => {
    if (!Array.isArray(pages)) return
    pages.forEach(p => {
      if (!p.href) {
        p.href = p.slug ? `/${p.slug}` : '/'
      }
      if (p.pageCoverThumbnail === undefined) {
        p.pageCoverThumbnail = ''
      }
    })
  }
  ensureHref(db.allPages)
  ensureHref(db.allNavPages)
  ensureHref(db.latestPosts)
  // 必须在使用完毕后才能进行清理
  ...
  // 最终清理：确保所有数据可被 JSON 序列化（undefined → null）
  sanitizeProps(db)
...
/**
 * 递归将对象中所有 undefined 值替换为 null，防止 Next.js getStaticProps 序列化失败
 */
