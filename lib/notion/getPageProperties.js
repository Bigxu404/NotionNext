...
export default async function getPageProperties(
  ...
  // Debug: print properties before mapping
  // console.log(`[getPageProperties] pre-map id: ${id}`, JSON.stringify({ type: properties.type, status: properties.status }, null, 2))

  // type\status\category 是单选下拉框 取数组第一个
  // Check if it's an array and if not just use the value directly, or handle string directly
  ...
  properties.category = (Array.isArray(properties.category) ? properties.category[0] : properties.category) || ''
  properties.comment = (Array.isArray(properties.comment) ? properties.comment[0] : properties.comment) || ''

  // Fallback to trying to get type and status directly from the raw property names if they exist and are single strings (for newer notion-client)
  if (!properties.type && properties.type !== '') {
     // fallback mapping handled below
  }

  // Find specific columns if they weren't mapped correctly by name
  if (!properties.type) {
    const rawType = value?.properties?.['`gQ~'] || value?.properties?.type || value?.properties?.['type'] || value?.properties?.['类型'];
    if (rawType) properties.type = getTextContent(rawType);
  }
  if (!properties.status) {
    const rawStatus = value?.properties?.['f211bdc0-ee00-4186-9a7d-f68c055ec2ee'] || value?.properties?.status || value?.properties?.['status'] || value?.properties?.['状态'];
    if (rawStatus) properties.status = getTextContent(rawStatus);
  }
  if (!properties.category) {
    const rawCategory = value?.properties?.['bB{M'] || value?.properties?.category || value?.properties?.['category'] || value?.properties?.['分类'];
    if (rawCategory) properties.category = getTextContent(rawCategory);
  }

  // Ensure type is Post if it was empty, but it has a title and a created date (fallback)
  if (!properties.type && properties.title) {
    properties.type = 'Post';
  }

  // Ensure status is Published if it was empty (fallback)
  if (!properties.status && properties.type === 'Post') {
  ...
  // console.log(`[getPageProperties] post-mapProperties id: ${id}`, JSON.stringify({ type: properties.type, status: properties.status }, null, 2))
