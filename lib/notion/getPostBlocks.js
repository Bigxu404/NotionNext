...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      // Unwrap the nested value.value structure from notion-client 7.7.1
      if (pageData && pageData.block) {
         Object.keys(pageData.block).forEach(blockId => {
            const b = pageData.block[blockId];
            if (b && b.spaceId && b.value && b.value.value) {
                // Replace the outer object with the inner one so it matches what react-notion-x expects
                pageData.block[blockId] = b.value;
            }
         });
      }
      
      if (pageData && pageData.collection) {
         Object.keys(pageData.collection).forEach(cId => {
            const c = pageData.collection[cId];
            if (c && c.spaceId && c.value && c.value.value) {
                pageData.collection[cId] = c.value;
            }
         });
      }
      
      if (pageData && pageData.collection_view) {
         Object.keys(pageData.collection_view).forEach(vId => {
            const v = pageData.collection_view[vId];
            if (v && v.spaceId && v.value && v.value.value) {
                pageData.collection_view[vId] = v.value;
            }
         });
      }
